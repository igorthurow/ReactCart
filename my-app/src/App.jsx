...
class App extends Component {
	...
	render() {
		return (
			<div className="App">
				<nav className="container-fluid navbar-app">
					<div className="row">
						<div className="col-md-6">
						</div>
						<div className="col-md-3 offset-md-3">
							<div className="cart-wrapper">
								<img src="https://www.essent.com/SiteData/SiteID34/Images/B2B-Ecommerce/B2B%20Ecommerce%202%20Cart%20Y.png"></img>
								<div className="cart"> {
									Object.keys(this.state.cartItems).length > 0 && (
										Object.keys(this.state.cartItems).map((item) => (
											<div className="cart-items">
												Preço: {this.state.cartItems[item].price.toFixed(2)}
												Nome: {this.state.cartItems[item].name}
												<button onClick={() => this.removeFromCart(this.state.cartItems[item].id)}></button>
											</div>
										))
									)}
								</div>
										...
										{(this.state.modal == item.name) && (
											<div
												...
												<div className="modal-container">
													...
													<h2 className="item-description">{item.description}</h2>
													<span className="item-price">R$ {item.price},00</span>
													<button onClick={() => { !this.sendToCart(item.stock, index) && alert('acabo') }}>Adicionar ao carrinho</button>
													<span className="item-button">
	...
	removeFromCart(id) {
		items[id].stock += this.state.cartItems[items[id].name].buyed
		delete this.state.cartItems[items[id].name]
		this.forceUpdate()
	}
		...
		if (stock > 0) {
			this.setState({
				cartItems: {
					...
					[items[index].name]: {
						buyed: this.state.cartItems[items[index].name]
							? this.state.cartItems[items[index].name].buyed + 1
							: 1,
						id: index,
